...
class MADESplit(nn.Module):
    ...
    def forward(self, inputs, cond_inputs=None, rev=False):
        if rev == False:
...
class LUInvertibleMM(nn.Module):
    ...
    def forward(self, inputs, cond_inputs=None, rev=False):
        ...
        if rev == False:
            return inputs[0] @ W, self.log_S.sum().unsqueeze(0).unsqueeze(
                0).repeat(inputs[0].size(0), 1)
        else:
            return inputs[0] @ torch.inverse(
...
class CouplingLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_inputs = num_inputs[0][0]
        self.mask = mask
...
class FlowSequential(nn.Sequential):
    ...
    def forward(self, inputs, cond_inputs=None, rev=False, logdets=None):
        ...
        if rev == False:
            for module in self._modules.values():
                inputs, logdet = module(inputs, cond_inputs, rev)
                logdets += logdet
        else:
